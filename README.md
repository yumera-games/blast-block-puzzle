...
| 3 | combo の条件を「盤面で隣接」→ **「起爆効果が実際に相手へ届いた」** | 隣接条件では 48,000 手で combo 14 回（起爆の 0.69%）しか成立せず、combo 系の効果が事実上の死に仕様だった |
...
同じ wave で起爆する特殊のうち、**片方の単独効果の範囲がもう片方のセルへ届いたもの**を
1 グループにまとめます（推移閉包を取るので 3 個以上も繋がります）。
Rainbow だけは効果が「対象色のセル全体」で通常セルしか含まないため、
**対象色を持つ特殊へは届く**ものとして扱います。
同じ wave に居るだけでは combo になりません。判定順序は index 昇順で決定論です。

## 既知の問題 / Phase 2B 以降への申し送り

- **重力が無いため、CHAIN は特殊の起爆でしか伸びない。** 仕様書の CHAIN の例をそのまま
  成立させたいなら、重力の有無自体が設計判断になる（Phase 2A では重力を入れていない）。
- **特殊の生成率が 4.49% → 7.46% へ増えた。** Rocket の生成閾値（BLAST 8）が
...
```bash
npm run test    # コアロジック 91 ケース（ブラウザ不要）
